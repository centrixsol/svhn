...
export default function PromptPanel({
  onGenerate, isGenerating, isRateLimited, generatingMsg,
  ...
  const handleGenerate = () => {
    if (!prompt.trim() || isGenerating || isRateLimited) return;
    ...
    <aside className="w-80 shrink-0 flex flex-col overflow-y-auto border-r border-white/5"
      ...
      <div className="p-5 flex flex-col gap-4">
          ...
          {uploadedImage ? (
            ...
            <div
              ...
              className={`rounded-xl border border-dashed flex flex-col items-center justify-center gap-2 py-7 cursor-pointer transition-all duration-200 ${
                dragOver ? 'border-violet-500/60 bg-violet-500/10' : 'border-white/10 hover:border-white/25 hover:bg-white/[0.02]'
              }`}>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}>
          {isGenerating ? (
            <><Loader2 size={16} className="animate-spin" />{generatingMsg}</>
          ) : isRateLimited ? (
            <><Loader2 size={16} className="opacity-40" />Quota reached — please wait</>
          ) : (
            <><Wand2 size={16} />Generate Image</>
          )}
        </motion.button>

        <p className="text-[10px] text-white/20 text-center">⌘ Enter to generate · Free with HF token</p>
